...
useEffect(()=>{
  ...
  const users = [
    ...
    {
      icon:pathProfile.concat("/1670122556968_295143059_1014464099222180_6719623139721774489_n.jpg"),
      email:"ju@gmail.com"
    }
  ]
        ...
        viewDropdownSchedule={viewDropdownSchedule}>
          ...
          <div className="owner--modal">
            ...
            <ul>
              {
              users
              .filter(key => owner.length > 0 ? key.email.toLowerCase().includes(owner) && key.email[0].toLowerCase() == owner[0] : '')
              .map((index) => {
                return <li
                onClick={() => {
                  setOwner("")
                ...
                ><img src={index.icon} />{index.email}</li>
              })
              }
            </ul>
          </div>
          ...
          <div className="options-calendar" onClick={() => {
